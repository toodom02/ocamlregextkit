...
(* |union| -- returns union of the two input lists *)
let rec union l1 l2 = 
    match l2 with
          [] -> l1
        | x::xs -> if not (List.mem x l1) then union (x::l1) xs else union l1 xs
...
let rec construct_rec_nfa re = 
    match re with
        ...
        | Union (r1, r2) -> let nfa1 = construct_rec_nfa r1 and nfa2 = construct_rec_nfa r2 in
                    counter := !counter + 1;
                    {states = (!counter - 1) :: (nfa1.states @ nfa2.states); alphabet = union nfa1.alphabet nfa2.alphabet; start = !counter - 1; accepting = nfa1.accepting @ nfa2.accepting; 
        ...
        | Concat (r1, r2) -> let nfa1 = construct_rec_nfa r1 and nfa2 = construct_rec_nfa r2 in
                    ...
                    {states = nfa1.states @ nfa2.states; alphabet = union nfa1.alphabet nfa2.alphabet; start = nfa1.start; accepting = nfa2.accepting;
