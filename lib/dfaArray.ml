...
type dfa = state AdtArray.automata

type product_op = Union | Intersection | SymmetricDifference

let get_states = AdtArray.get_states
let get_alphabet = AdtArray.get_alphabet
let get_transitions = AdtArray.get_transitions
let get_start = AdtArray.get_start
let get_accepting = AdtArray.get_accepting

(* |is_accepting| -- returns true if state s is accepting *)
let is_accepting = AdtArray.is_accepting

(* |succ| -- the resulting state of dfa m after reading symbol *)
let succ m s a = List.hd (AdtArray.get_next_states m s a)

(* |pred| -- returns the state preceeding state in dfa m before reading symbol *)
let pred = AdtArray.get_prev_states
...
let export_graphviz d =
    ...
    (List.fold_left (fun a s -> 
        let shape = "ellipse, " ^ if AdtArray.is_accepting d s then "peripheries=2, " else "" in
...
let complement m = 
    AdtArray.create_automata (get_states m) (get_alphabet m) (get_transitions m) (get_start m) (List.filter (fun s -> not (is_accepting m s)) (get_states m))

(* |reachable_states| -- returns the set of reachable states in dfa m *)
let reachable_states = AdtArray.get_reachable_states

(* |prune| -- reduces input dfa by pruning unreachable states *)
let prune m = 
    let marked = reachable_states m in
    AdtArray.create_automata (List.filter (fun s -> List.mem s marked) (get_states m)) (get_alphabet m) (List.filter (fun (s,_,_) -> List.mem s marked) (get_transitions m)) (get_start m) (List.filter (fun s -> List.mem s marked) (get_accepting m))
...
let product_construction op m1 m2 =
    ...
    AdtArray.create_automata cartesianStates unionAlphabet cartTrans (ProductState (get_start m1, get_start m2)) cartAccepting
...
let disjoin_dfas m1 m2 =
    ...
    (
        AdtArray.create_automata (get_states m1) merged_alphabet (get_transitions m1) (get_start m1) (get_accepting m1),
        AdtArray.create_automata (List.rev_map (fun s -> negate_state s) (get_states m2)) merged_alphabet (List.rev_map (fun (s,a,t) -> (negate_state s,a,negate_state t)) (get_transitions m2)) (negate_state (get_start m2)) (List.rev_map (fun s -> negate_state s) (get_accepting m2))
    )
...
let myhill_min m =
    ...
    AdtArray.create_automata merged_states (get_alphabet m') newtrans newstart newaccepting
...
let brzozowski_min m =
    let reverse_and_determinise d =
        ...
        AdtArray.create_automata !newstates (get_alphabet d) !newtrans (State newstart) newaccepting
    in
    (* reverse DFA *)
...
let nfa_to_dfa (n: NfaArray.nfa) =
    let newstart = NfaArray.eps_reachable_set n [get_start n] in
    ...
    while (List.length !stack > 0) do
        ...
        stack := List.tl !stack;
        List.iter (fun a ->
            let nextstate = List.concat_map (fun s -> AdtArray.get_next_states n s a) currentstate in
            let epsnext = NfaArray.eps_reachable_set n nextstate in
    ...
    let newaccepting = List.filter (function
        | State s -> List.exists (NfaArray.is_accepting n) s
        | _ -> false
    ) !newstates in

    AdtArray.create_automata !newstates (get_alphabet n) !newtrans (State newstart) newaccepting
...
let create qs alph tran init fin =
    ...
    AdtArray.create_automata newstates alph newtrans newinit newfin