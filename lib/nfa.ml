...
let export_graphviz n =
  Printf.sprintf
    ...
    (List.fold_left
       (fun a s ->
         let shape =
           "ellipse, " ^ if is_accepting n s then "peripheries=2, " else ""
         in
...
let succ n state symbol =
  ...
  let succs =
    List.fold_right Utils.add_unique
      (List.concat_map
         (fun s -> Adt.get_next_states n s symbol)
         initial_reachable)
      []
  in
...
let get_accepted n =
  let queue =
    ref (List.rev_map (fun s -> (s, "")) (eps_reachable_set n [ get_start n ]))
  and seen = ref []
      ...
      and newt =
        List.concat_map
          (fun a ->
            ...
            List.filter_map
              (fun t ->
                if not (List.mem t !seen) then Some (t, currentWord ^ a)
                else None)
              nexts)
          (get_alphabet n)
      in
...
let create qs alph tran init fin =
  ...
  if not (List.mem init qs) then
    raise (Invalid_argument "NFA Initial State not in States");
  List.iter
    (fun f ->
      if not (List.mem f qs) then
        raise (Invalid_argument "NFA Accepting State not in States"))
    fin;
  List.iter
    (fun (s, a, t) ->
...
let rec construct_rec_nfa = function
  ...
  | Union (r1, r2) ->
      ...
      and transitions =
        ((!counter - 1, "ε", i1) :: t1) @ ((!counter - 1, "ε", i2) :: t2)
      and start = !counter - 1
...
let re_to_nfa re =
  ...
  Adt.create_automata (List.sort compare states) (List.rev alphabet) transitions
    start accepting
