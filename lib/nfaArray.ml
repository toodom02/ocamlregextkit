...
type state = int
type nfa = state AdtArray.automata
...
let eps_reachable_set n ss =
    ...
    let get_reachable_set states =
        List.fold_right (fun s acc -> Utils.list_union acc (AdtArray.get_next_states n s "ε")) states states
    in
...
let succ n state symbol =
    ...
    let succs = List.fold_right Utils.add_unique (List.concat_map (fun s -> AdtArray.get_next_states n s symbol) initial_reachable) [] in
...
let pred n state =
    ...
    let preds = List.fold_left (fun acc s ->
            List.fold_left (fun acc' a ->
                Utils.list_union acc' (AdtArray.get_prev_states n s a)
            ) acc (get_alphabet n)
        ) [] epspreds in
...
let prune n = 
    ...
    AdtArray.create_automata (List.filter (fun s -> List.mem s marked) (get_states n)) (get_alphabet n) (List.filter (fun (s,_,_) -> List.mem s marked) (get_transitions n)) (get_start n) (List.filter (fun s -> List.mem s marked) (get_accepting n))
...
let get_accepted n =
    ...
    while Option.is_none !shortest && List.length !queue > 0 do
        ...
        if (is_accepting n currentState) then (shortest := Some(currentWord))
            ...
            ) (AdtArray.get_next_states n currentState "ε") and
                newt = List.concat_map (fun a -> 
                let nexts = AdtArray.get_next_states n currentState a in
...
let merge_alphabets n1 n2 =
    ...
    (
        AdtArray.create_automata (get_states n1) newalphabet (get_transitions n1) (get_start n1) (get_accepting n1),
        AdtArray.create_automata (get_states n2) newalphabet (get_transitions n2) (get_start n2) (get_accepting n2)
    )
...
let create qs alph tran init fin =
    ...
    AdtArray.create_automata newstates alph newtran newinit newfin
...
let re_to_nfa re = 
    ...
    AdtArray.create_automata (List.sort compare states) (List.rev alphabet) transitions start accepting