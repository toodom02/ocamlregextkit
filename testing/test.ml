  ...
  let transition =
    ...
    while List.length !unconnected > 0 do
      ...
      let eligable_states =
        List.filter
          (fun s ->
            not
              (List.for_all
                 (fun a -> List.exists (fun (s', a', _) -> s' = s && a' = a) !tran)
                 alphabet))
          !connected
      in
      let randconnected =
      ...
      let eligable_letters =
        List.filter
          (fun a ->
            not (List.exists (fun (s, a', _) -> s = randconnected && a = a') !tran))
          alphabet
      in
      let randletter =
  ...
  let str =
    List.init
      ...
      (fun i ->
        ...
        then
          if Random.float 1. < 0.2 && !bracks > 0
          then (
            decr bracks;
            ")")
          else ""
        else if i mod 5 = 3
        then if Random.float 1. < 0.1 then "*" else ""
        else if i mod 5 = 4
        then List.nth [ "+"; "." ] (Random.int 2)
        else if Random.float 1. < 0.2
        then (
          incr bracks;
          "(")
        else "")
...
let _construction_tester () =
  ...
  Printf.printf
    "Size, Total Nfa->Dfa, Total Brzozowski, Nfa->Dfa, Brzozowski, Minimal, Minimal \
     States, Mean Absolute Error\n";
  let iters = 100 in
  for s = 1 to 50 do
    ...
    for _ = 1 to iters do
      ...
      if not (Dfa.is_equiv minimal dfa2)
      then (
        print_string "Failed\n";
        exit 1);
...
let _equiv_tester () =
  ...
  for s = 1 to 20 do
    ...
    for _ = 1 to iters do
      let d1 = generate_random_dfa s
      and d2 = generate_random_dfa s in
...
let _circular_equiv_tester () =
  ...
  for s = 1 to 20 do
    let cumul_time_hopcroft = ref 0.
    and cumul_time_symmetric = ref 0. in
    for _ = 1 to iters do
...
let _min_tester () =
  ...
  Printf.printf
    "# States,Total Myhill,Total Hopcroft,Total Brzozowski,Myhill,Hopcroft,Brzozowski\n";
  let iters = 1000 in
  for s = 1 to 20 do
    ...
    for _ = 1 to iters do
      ...
      (* Sanity check that our results are the same *)
      if (not (Dfa.is_equiv d res_1))
         || (not (Dfa.is_equiv res_1 res_2))
         || not (Dfa.is_equiv res_2 res_3)
      then (
        print_string "Failed\n";
        exit 1);
      if (not (List.length (Dfa.get_states d) >= List.length (Dfa.get_states res_1)))
         || (not
               (List.length (Dfa.get_states res_1) = List.length (Dfa.get_states res_2)))
         || not (List.length (Dfa.get_states res_2) = List.length (Dfa.get_states res_3))
      then (
        print_string "Failed\n";
        exit 1);
      if (not
